  ...
  render() {
    ...
    thumb.addEventListener('pointerdown', (event) => {
      ...
      function onMouseMove(event) {
        ...
        let approximateValue = leftRelative * (thSteps - 1);
        this.value = Math.round(approximateValue);
        let sldValue = elem.querySelector('.slider__value');
        sldValue.textContent = this.value;

        let sldSpan = (elem.querySelector('.slider__steps')).querySelectorAll('span');
        for (let i = 0; i < sldSpan.length; i++) {
          if (sldSpan[i].classList.contains('slider__step-active')) {
            sldSpan[i].classList.remove('slider__step-active');     
          };
